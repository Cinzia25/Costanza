...
def compute_cmd(idx, H, T, O, logger, is_herder):
    ...
    for h in H.values():
        if h is not None:
            positions.append([h.x_trans, h.y_trans])
    for t in T.values():
        if t is not None:
    ...
    pose_o = O.get(1)
    xc, yc = pose_o.x_trans, pose_o.y_trans
    xc = xc/1000
    yc = yc/1000
    theta_rad = (pose_o.z_rot_euler)
    obs = rotated_box(center=(xc, yc), size=(lx, ly), angle_rad=theta_rad)
    ...
    if is_herder:
        ...
        xh, yh = pose_h.x_trans, pose_h.y_trans
        xh = xh/1000
        yh = yh/1000
        herder_pos = np.array([xh, yh])
        yaw = pose_h.z_rot_euler
        point_h = Point(xh, yh)    
        ...
        if not T:
            ...
            T_array = np.array([[t.x_trans, t.y_trans] for t in T.values()])/1000
            H_array = np.array([[h.x_trans, h.y_trans] for h in H.values()])/1000
        ...
        xt, yt = pose_t.x_trans, pose_t.y_trans
        xt = xt/1000
        yt = yt/1000
    ...
    if is_herder:
        current_pos = np.array([H[idx].x_trans, H[idx].y_trans])/1000
    else:
        current_pos = np.array([T[idx].x_trans, T[idx].y_trans])/1000
